...
export const useMap = (map: Ref<Map>) => {
  ...
  const drawFlightsOnMap = async (flights: Ref<any[]>) => {
    ...
    for (const trip of flights.value) {
      ...
      if (!fromMarkers.includes(trip.from.icao)) {
        fromMarker.addTo(map.value).bindPopup(createPopupText(flightsDetails.value[trip.from.icao]));
      }

      if (!toMarkers.includes(trip.to.icao)) {
        toMarker.addTo(map.value).bindPopup(createPopupText(flightsDetails.value[trip.to.icao]));
      }
    ...
    await map.value.setView(KATOWICE_LAT_LON, 4);
  };
  ...
  return {
    drawFlightsOnMap, initializeMap
  };
};
